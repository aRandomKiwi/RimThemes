...
namespace aRandomKiwi.RimThemes
{
    ...
    [HarmonyPatch(typeof(Widgets), "ButtonImage", new Type[] { typeof(Rect), typeof(Texture2D), typeof(Color), typeof(Color), typeof(bool), typeof(string) }), StaticConstructorOnStartup]
    class Widgets_ButtonImage_Patch
    {
        [HarmonyPrefix]
        static bool Prefix(Rect butRect, Texture2D tex, Color baseColor, Color mouseoverColor, bool doMouseoverSound, string tooltip, ref bool __result)
        {
            if (!(Themes.initialized && Themes.vanillaThemeSaved))
                return true;
